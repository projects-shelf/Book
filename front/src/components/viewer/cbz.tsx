...
export function CBZViewer({ fileUrl, initialPage = 1 }: CBZViewerProps) {
    ...
    useEffect(() => {
        const fetchPageCount = async () => {
            try {
                const res = await fetch(`/book/cbz/pages?path=${encodedFilePath}`);
                ...
                console.error("Error fetching CBZ page count:", err);
            }
        };
                ...
                <div style={{ display: "flex", justifyContent: 'center', transform: `scale(${scale})`, transformOrigin: 'top center' }}>
                    {direction === 'rtl' && isSpreads && (
                        <img style={{ width: "auto", height: "auto", maxWidth: "none", maxHeight: "none" }}
                            key={`l-/book/cbz?path=${encodedFilePath}&page=${Math.min(standerisedPageNumber + 1, numPages ?? 1)}`}
                            src={`/book/cbz?path=${encodedFilePath}&page=${Math.min(standerisedPageNumber + 1, numPages ?? 1)}`} />
                    )}
                    <img style={{ width: "auto", height: "auto", maxWidth: "none", maxHeight: "none" }}
                        key={`m-/book/cbz?path=${encodedFilePath}&page=${standerisedPageNumber}`}
                        src={`/book/cbz?path=${encodedFilePath}&page=${standerisedPageNumber}`}
                        ref={imgRef} onLoad={() => {
                    ...
                    {direction === 'ltr' && isSpreads && (
                        <img style={{ width: "auto", height: "auto", maxWidth: "none", maxHeight: "none" }}
                            key={`r-/book/cbz?path=${encodedFilePath}&page=${Math.min(standerisedPageNumber + 1, numPages ?? 1)}`}
                            src={`/book/cbz?path=${encodedFilePath}&page=${Math.min(standerisedPageNumber + 1, numPages ?? 1)}`} />
                    )}
                    ...
                    <div style={{ position: 'absolute', width: 0, height: 0, overflow: 'hidden' }}>
                        <img style={{ width: "auto", height: "auto", maxWidth: "none", maxHeight: "none" }}
                            src={`/book/cbz?path=${encodedFilePath}&page=${Math.max(standerisedPageNumber - 1, 1)}`} />
                        <img style={{ width: "auto", height: "auto", maxWidth: "none", maxHeight: "none" }}
                            src={`/book/cbz?path=${encodedFilePath}&page=${Math.max(standerisedPageNumber - 2, 1)}`} />
                        <img style={{ width: "auto", height: "auto", maxWidth: "none", maxHeight: "none" }}
                            src={`/book/cbz?path=${encodedFilePath}&page=${Math.min(standerisedPageNumber + 1, numPages ?? 1)}`} />
                        <img style={{ width: "auto", height: "auto", maxWidth: "none", maxHeight: "none" }}
                            src={`/book/cbz?path=${encodedFilePath}&page=${Math.min(standerisedPageNumber + 2, numPages ?? 1)}`} />
                        <img style={{ width: "auto", height: "auto", maxWidth: "none", maxHeight: "none" }}
                            src={`/book/cbz?path=${encodedFilePath}&page=${Math.min(standerisedPageNumber + 3, numPages ?? 1)}`} />
                    </div>
                    {/* Shadow Page */}
